...
@media(max-width:760px){.wc2col{grid-template-columns:1fr;gap:12px;margin-bottom:14px}.wc2col .wccatimg{aspect-ratio:3/2;max-height:210px}#wcpanel .wcmcards{grid-template-columns:1fr 1fr}}
...
function render(a){
  ...
  if(c.img && c.prods.length===1){
    // single product: left = image (half), right = product card
    panel.innerHTML=intro+'<div class="wc2col">'+img+'<div class="wcright">'+card(c.prods[0])+'</div></div>';
  }else if(c.img){
    // multiple products: left = image (half), right = description; cards below
    ...
    panel.innerHTML=intro+'<div class="wcmcards">'+c.prods.map(card).join('')+'</div>';
  }
}
